...
def get_document(
    ...
    if doc.tenant_id != current_user.tenant_id:
        raise HTTPException(status_code=403, detail="No podés VER/EDITAR/BORRAR documentos de otra empresa")
...
def delete_document(
    ...
    _: User = Depends(require_scope("write")),  # 🔓 TODO: Depends(require_role(Role.ADMIN)) + Depends(require_scope("write"))
):
